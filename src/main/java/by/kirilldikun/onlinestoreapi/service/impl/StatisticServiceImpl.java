...
public class StatisticServiceImpl implements StatisticService {
    ...
    private <T extends Chart<?, ?>> byte[] writeChartToPNG(T chart) {
    ...
    @Override
    @Transactional(readOnly = true)
    public byte[] createCategoriesRevenueChart() {
    ...
    @Override
    @Transactional(readOnly = true)
    public byte[] createUsersExpensesChart(int limit) {
        List<UserExpensesDto> userExpensesDtos = userRepository.getUsersExpenses(PageRequest.ofSize(limit));
        List<String> usernames = userExpensesDtos
                .stream()
                .map(UserExpensesDto::username)
                .toList();
        List<BigDecimal> expenses = userExpensesDtos
                .stream()
                .map(UserExpensesDto::totalExpenses)
                .toList();
        CategoryChart chart = new CategoryChartBuilder()
                .width(800)
                .height(600)
                .title("Топ %d пользователей по общим тратам".formatted(limit))
                .xAxisTitle("Пользователь")
                .yAxisTitle("Общие траты")
                .build();
        chart.addSeries("Пользователи", usernames, expenses);
        return writeChartToPNG(chart);
    }
}
